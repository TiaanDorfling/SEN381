...
const App: React.FC = () => {
  ...
  const fetchAndSetGreeting = async () => {
    setIsLoading(true);
    try {
  ...
  }, []); // Intentionally empty to run only on initial mount if history is empty
  ...
  const handleClearConversation = () => {
    if (window.confirm("Are you sure you want to clear the entire conversation? This action cannot be undone.")) {
      // Setting messages to an empty array will trigger the useEffect to save
      // the empty history to localStorage and the other useEffect to fetch a new greeting.
      setMessages([]);
      fetchAndSetGreeting();
    }
  };
